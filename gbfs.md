...
&emsp;- discovery_uri | Conditionally Required | A URI that can be used to discover if the rental Android app is installed on the device (e.g., using PackageManager.queryIntentActivities()). This intent is used by viewing apps prioritize rental apps for a particular user based on whether they already have a particular rental app installed. This field is required if a rental_uris.android field is populated, otherwise it is optional. Example value: com.abcrental.android://. See the [Deep Links](#Deep-Links) section for parameters.
...
&emsp;- discovery_uri  | Conditionally Required | A URI that can be used to discover if the rental iOS app is installed on the device (e.g., using [UIApplication canOpenURL:]). This intent is used by viewing apps prioritize rental apps for a particular user based on whether they already have a particular rental app installed. This field is required if a rental_uris.ios field is populated, otherwise it is optional. Example value: com.abcrental.ios://. See the [Deep Links](#Deep-Links) section for parameters.
...
rental_uris        | Optional  | A JSON object that contains rental URIs for Android, iOS, and web in the android, ios, and web fields. See the [Deep Links](#Deep-Links) section for parameters.       
